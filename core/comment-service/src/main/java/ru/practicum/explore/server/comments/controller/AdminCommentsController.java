...
public class AdminCommentsController {
    ...
    public Collection<FullCommentResponseDto> geComments(@Valid
                                                         ...
                                                         Integer size) {
        GetAdminCommentsFilter parsedFilter = GetAdminCommentsFilter.fromParam(filter);
        log.info("Запрос комментариев администратором: from={}, size={}, filter={}", from, size, parsedFilter);
        if (size < 1) {
    ...
    public void setApprovedOrRejectComment(@PathVariable Long commentId,
                                           @RequestParam(name = "status") String status) {
        commentsService.approveOrRejectComment(commentId, CommentStatusAction.fromParam(status));
    }
